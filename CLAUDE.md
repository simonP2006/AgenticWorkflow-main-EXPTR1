...
│   │   ├── validate_workflow.py           (Workflow.md DNA Inheritance P1 검증 독립 스크립트 — W1-W5 구조적 무결성, JSON 출력, Hook 아님 — workflow-generator 완료 후 수동 호출)
...
- **Workflow.md DNA Inheritance P1 검증**: `validate_workflow_md()`가 생성된 workflow.md의 DNA 유전 구조적 무결성을 5항목(W1 파일 존재, W2 최소 크기 500 bytes, W3 `## Inherited DNA` 헤더 존재, W4 Inherited Patterns 테이블 ≥ 3행, W5 Constitutional Principles 섹션 존재)으로 검증. 독립 실행 스크립트: `validate_workflow.py`. workflow-generator 완료 후(SKILL.md Step 13) 수동 호출.
