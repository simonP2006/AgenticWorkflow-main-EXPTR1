...
def _ar_reanchor(xlsx_path):
    """★Determinism-reduction hook (2026-06-24): invoke the canonical aspect-ratio-preserving re-anchor
    (scripts/fix_receipt_anchors.py — a permanent suite component, same import pattern as write_excel).
    Converts the just-injected stretch twoCellAnchors → AR-preserving/orientation-aligned oneCellAnchors
    IN PLACE (pixels untouched). Returns the report dict (pixels_unchanged + per-receipt squish/orient)."""
    sys.path.insert(0, str(PROJECT_DIR / "scripts"))
    import fix_receipt_anchors as _fra
    return _fra.reanchor(xlsx_path, xlsx_path)


def _ar_verify(xlsx_path):
    """★Non-vacuous independent gate: re-reads the placed workbook and FAILs if ANY receipt is still a
    stretched twoCellAnchor / has no ext / wrong orientation / is EXIF-rotated / or there are zero receipt
    pics. Counts surviving defects directly from the file (does NOT trust the reanchor report — closes the
    'all([])==True' vacuous-pass hole)."""
    sys.path.insert(0, str(PROJECT_DIR / "scripts"))
    import fix_receipt_anchors as _fra
    return _fra.verify(xlsx_path, verbose=False)
...
def zip_drawing_counts(xlsx_path):
    ...
    pics = sps = anchors = one_anchors = sp_rdr = 0
    drawings = []
    with zipfile.ZipFile(xlsx_path) as z:
        for n in z.namelist():
            if n.startswith("xl/drawings/drawing") and n.endswith(".xml"):
                ...
                one_anchors += len(re.findall(r"<\w*:?oneCellAnchor\b", xml))   # ★AR-FIX: receipts now ship as oneCellAnchor
    ...
    return {"drawings": drawings, "pic": pics, "sp": sps, "anchor": anchors,
            "one_anchor": one_anchors, "sp_rdr": sp_rdr}
...
def _to_png_bytes(img_path, allow_grayscale=False):
    ...
    if p.lower().endswith(".pdf"):
        ...
        p = pdf_to_png(p, tmp)
    img = Image.open(p)
    # ★EXIF-FIX (§5 CRITICAL, 2026-06-24): bake EXIF orientation into the pixels BEFORE embedding. A phone
    # receipt with EXIF Orientation=6/8 is stored sideways (landscape buffer) with an orientation tag; PNG
    # drops the tag, so without this the receipt embeds SIDEWAYS and the .size-based AR gate (also EXIF-blind)
    # passes it = the 'lying down' defect re-introduced. exif_transpose returns an upright image, tag cleared.
...
def physical_verify(xlsx_path, expected_pics_added, baseline):
    ...
    # ★AR-FIX: receipts ship as oneCellAnchor (not twoCellAnchor) after the AR re-anchor, and a template
    # pic twoCellAnchor may convert to oneCellAnchor — so "anchors not lost" must count BOTH anchor kinds
    # (total conserved/grown), not twoCellAnchor alone (else a converted pic reads as a lost anchor).
    checks["anchors_not_lost"] = (after["anchor"] + after["one_anchor"]) >= (baseline["anchor"] + baseline["one_anchor"])
...
def place(template_xlsx, output_xlsx, placements, drawing=None):
    ...
    try:
        ...
        # ★AR-FIX (determinism-reduction, 2026-06-24): the twoCellAnchor boxes injected above STRETCH each
        # image to fill its cell box (root cause of the WK23 distortion/lying-down defect). Immediately
        # convert them to aspect-ratio-preserving, orientation-aligned oneCellAnchors — pixels untouched.
        # The rendered shape is now a pure deterministic function of (native_w, native_h, box): cx/cy ==
        # native_w/native_h EXACTLY (no LLM, no fill-to-box). See scripts/fix_receipt_anchors.py.
        ar_rep = _ar_reanchor(candidate)
        # ★NON-VACUOUS fail-closed AR gate (closes the 'all([])==True' hole the §5 panel found):
        #   (1) pixels byte-identical, (2) reanchor actually processed ≥ the n placed pics (not a silent no-op),
        #   (3) every reported receipt is AR-exact + orientation-preserved + NOT EXIF-rotated, AND
        #   (4) the INDEPENDENT verify() re-reads the file and finds 0 surviving stretched/EXIF-rotated pics.
        ar_independent = _ar_verify(candidate)       # counts defects directly from the workbook (not the report)
        ar_ok = (bool(ar_rep["pixels_unchanged"])
                 and (n == 0 or len(ar_rep["receipts"]) >= n)
                 and all(r["squish_pct"] <= 0.01 and r["native_orient"] == r["display_orient"]
                         and not r.get("exif_rotated") for r in ar_rep["receipts"])
                 and ar_independent)
        v = physical_verify(candidate, n, baseline)
        v["ar_preserved"] = ar_ok                    # ★fail-closed gate: no stretched/flipped/sideways image may ship
        v["ar_independent_verify"] = ar_independent
        v["pixels_unchanged"] = bool(ar_rep["pixels_unchanged"])
        if not (v["valid"] and ar_ok):               # AR gate is part of the adoption gate (was the escape hatch)
            candidate.unlink(missing_ok=True)
            reason = "post-placement physical verify FAILED" if not v["valid"] else \
                     "AR/orientation gate FAILED (stretched or flipped image would ship)"
            return {"status": "ESCALATE", "reason": reason,
                    "verify": {k: v[k] for k in v if not k.startswith("_")},
                    "ar_receipts": ar_rep["receipts"], "placed": 0}
        os.replace(candidate, output_xlsx)           # atomic adoption only after PASS (physical + AR)
        return {"status": "OK", "placed": n, "output": str(output_xlsx),
                "verify": {k: v[k] for k in v if not k.startswith("_")},
                "ar_receipts": ar_rep["receipts"]}
    except Exception as e:
...
def _selftest():
    ...
    # ★§5-CRITICAL EXIF: _to_png_bytes bakes EXIF orientation into pixels → an EXIF=6 phone photo embeds UPRIGHT
    _exif6 = bd / "exif6.jpg"
    _ei = _BImg.new("RGB", (2000, 600), (40, 40, 160)); _ex6 = _ei.getexif(); _ex6[274] = 6; _ei.save(_exif6, exif=_ex6)
    _exup = _BImg.open(io.BytesIO(_to_png_bytes(str(_exif6)))).size
    check("★§5-CRITICAL: _to_png_bytes exif_transpose → EXIF=6 (2000×600) embeds UPRIGHT (600×2000), not sideways",
          _exup == (600, 2000))
    ...
    if _tmplc:
        ...
        _FW = {"TELEPHONE-LOCAL", "PARKING/TOLLS"}        # full-width per_row=1 sectors → cols [0]/span 28 (by design)
        check("★detect_sector_bands: each band carries the CORRECT calibrated geometry per type "
              "(3-grid [0,10,20]/8 · full-width [0]/28 · all row_span 63)",
              len(bands) >= 3 and all(
                  b["row_span"] == 63 and (
                      (b["cols"] == [0] and b["col_span"] == 28) if name in _FW
                      else (b["cols"] == [0, 10, 20] and b["col_span"] == 8))
    ...
    # ★AR-FIX gate: place() now ships AR-preserving/orientation-aligned oneCellAnchors (not stretch boxes)
    check("★AR-FIX: place() verify reports ar_preserved=True", res.get("verify", {}).get("ar_preserved") is True)
    check("★AR-FIX: place() reports pixels_unchanged=True", res.get("verify", {}).get("pixels_unchanged") is True)
    check("★AR-FIX (§5-MEDIUM): independent verify() gate ran + passed (non-vacuous, not just the report)",
          res.get("verify", {}).get("ar_independent_verify") is True)
    check("★AR-FIX: every placed receipt squish ≤ 0.01% (exact AR) + orientation preserved",
          all(r["squish_pct"] <= 0.01 and r["native_orient"] == r["display_orient"] for r in res.get("ar_receipts", [])))
    check("★AR-FIX NON-VACUOUS: ar_receipts has 2 entries (gate actually inspected the placed images)",
          len(res.get("ar_receipts", [])) == 2)
    ...
    check("실측: oneCellAnchor count grew by 2 (AR-preserving anchors added; pics not destroyed)",
          after["one_anchor"] - baseline["one_anchor"] == 2)
